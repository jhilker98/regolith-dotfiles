...
I keep my agenda files in Dropbox so that I can access them on any computer. In addition, I want a different ellipsis instead of the three periods. Most of my todo keywords are fairly straightforward.
#+begin_src emacs-lisp
(setq org-agenda-files '("~/Dropbox/org/agenda.org" "~/Dropbox/org/projects.org" "~/Dropbox/org/inbox.org") 
        org-ellipsis " ▼ "
        org-todo-keywords '(
                            (sequence "TODO(t)" "NEXT(n)" "MEETING(m)" "HOLD(h)" "REVIEW(r)" "SOMEDAY(s)" "|" "DONE(d!)" "CANCELLED(c!)")
                            (type "DRAFT(D)" "PROJECT(p)")
                            )
        ...
        org-todo-keyword-faces '(
                                 ...
                                 ("REVIEW" . (:foreground "#b16286" :weight bold))
                                 ("SOMEDAY" . (:foreground "#b16286" :slant italic))
                                 ("DONE" . (:foreground "#83a598" :strike-through t))
                                 ("CANCELLED" . (:foreground "cc241d" :strike-through t))
                                 ("DRAFT" . (:foreground "#83a598" :weight bold))
                                 ("PROJECT" . (:foreground "#458588" :slant italic :weight bold))))
        #+end_src
...
*** Appointment
I want to have a capture template for immediately being able to add an appointment to my agenda.
#+begin_src emacs-lisp
...
*** TODO Mail
I want to be able to add stuff to the mail section of my agenda.
#+begin_src emacs-lisp
...
***** Planner 
This provides an overview of my day and the things I need to get done for the day, as well as anything coming up in the next 2 days.
#+begin_src emacs-lisp
