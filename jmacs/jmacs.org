...
*** Initial Setup
This is a very basic function that sets up org-mode to use visual line mode, org-indent mode, and variable-pitch mode - just a few minor ui tweaks. 
#+begin_src emacs-lisp
...
(add-hook 'org-mode-hook 'jh/org-agenda-setup)

(defvar jh/agenda-block-separator
  (concat (make-string (window-width) 9472) "\n")
  "A nicer block separator for the agenda")
...
(setq org-capture-templates
      (doct-add-to org-capture-templates '(("Appointment"
               :keys "a"
               :file "c:/Users/camoh/Dropbox/org/agenda.org"
...
(setq org-capture-templates
      (doct-add-to org-capture-templates
                   '("Mail" 
                     :keys "m"
                     :file "c:/Users/camoh/Dropbox/org/todo.org"
                     :headline "Mail"
                     :children(("Follow Up" 
                                :keys "f"
                                :headline "Follow Up"
...
*** Tasks
I want to be able to quickly add tasks and calls to my inbox file.
#+begin_src emacs-lisp
(setq org-capture-templates
      (doct-add-to org-capture-templates
                   '("Task" 
                     :keys "t"
                     :file "c:/Users/camoh/Dropbox/org/inbox.org"
                     :children(("Task" 
                                :keys "t"
                                :template ("* TODO %^{Description}"
                                           ":PROPERTIES:"
...
** TODO Refiling
I want to set up refiling here. As a main note, I want to archive everything under an “Archive” headline under the main context (such as Work, Personal, etc.) I want to also enable caching, since I will have a fair number of headlines. I am also using a hydra and a custom function (originally from [[https://mollermara.com/blog/Fast-refiling-in-org-mode-with-hydras/][this blog post]]) for handling my refiling for different files.
#+begin_src emacs-lisp
...
(defun jh/refile (file headline &optional arg)
...
(defhydra jh/org-refile-hydra (:foreign-keys run) 
  "Refile"
  ("t" (jh/refile "~/Dropbox/org/todo.org" "Tasks") "Refile to Task List")
  ("q" nil "Cancel"))
#+end_src

** TODO Archiving
When I archive something, I want to have it go to =archive.org= in my Dropbox, under the relevant headline.
#+begin_src emacs-lisp
