...
for num in range(0,8):
  td_cond = np.append(td_cond,weather[0]['hourly'][num]['weatherDesc'][0]['value'].lower())
...
tm_cond = np.array([])
for num in range(0,8):
  tm_cond = np.append(tm_cond,weather[0]['hourly'][num]['weatherDesc'][0]['value'].lower())
tmcnd, tm_cond_cnt = np.unique(tm_cond,return_counts=True)
tmct = np.argsort(-tm_cond_cnt)

nd_cond = np.array([])
for num in range(0,8):
  nd_cond = np.append(td_cond,weather[0]['hourly'][num]['weatherDesc'][0]['value'].lower())
